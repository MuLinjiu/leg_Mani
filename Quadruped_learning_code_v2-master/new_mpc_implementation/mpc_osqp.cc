...
void CalculateAMat(const Eigen::Vector3d& rpy,
                   const Eigen::Vector3d& ground_normal_vec,
...
void CalculateAMat(const Vector3d& rpy, const Vector3d& ground_normal_vec,
                   MatrixXd* a_mat_ptr) {
  ...
  a_mat(4, 10) = 1;
  a_mat(5, 11) = 1;
  a_mat(9, 12) = ground_normal_vec[0];
  a_mat(10, 12) = ground_normal_vec[1];
  a_mat(11, 12) = ground_normal_vec[2];
}
...
std::vector<double> ConvexMpc::ComputeContactForces(
  ...
  const Vector3d ground_normal(ground_normal_vec[0], ground_normal_vec[1],
                               ground_normal_vec[2]);

  CalculateAMat(rpy, ground_normal, &a_mat_);
