...
class QuadrupedGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        if self._render:
            ...
            print("Des vel: ", self._des_velocity_x)
    ...
    def _getCurrentObservation(self):
        ...
        # obs_noise = np.random.normal(size=observation.shape) * self._obs_noise_scale
    ...
    def step(self, action):
        ...
        for _ in range(self._action_repeat):
            ...
            self._robot.ApplyAction(kpJoint, kdJoint, qDes, np.zeros(12), tau, add_noise=self._domain_randomization)
            self._pybullet_client.stepSimulation()
    ...
    def get_reward(self):
        ...
        vel_reward = .2 * (2 - (abs(base_vel[0] - des_vel_x_world_frame) + abs(base_vel[1] - des_vel_y_world_frame)))
        # if vel_reward < 0:
        #   vel_reward = 0
        # orn_weight = 1e-3
        orn_weight = 0.2  # 1e-2
        ...
        yaw_rate_reward = -0.08 * (abs(yaw_rate_act - self._des_yaw_rate))
        # y_reward = -0.1 * abs(self._robot.GetBasePosition()[1])
        # print(yaw_rate_act)
        height_reward = -0.2 * abs(self._robot.GetBasePosition()[2] - 0.28)

        feetInContactBool = self._robot.GetFootContacts()
        feet_in_air_weight = 0.05
        ...
        return vel_reward + energy_reward + yaw_rate_reward + feet_in_air_reward \
            + height_reward + orn_reward + survival_reward
